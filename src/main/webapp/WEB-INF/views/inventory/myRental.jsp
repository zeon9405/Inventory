...
<body>
	...
	<div class="container">
		<div class="d-flex justify-content-between align-items-center mb-4">
			...
			<a href="/" class="btn btn-outline-secondary shadow-sm"> 메인페이지로
				돌아가기 </a>
		</div>
				...
				<table class="table table-hover align-middle">
					...
					<tbody>
						<c:forEach var="rental" items="${myRentals}">
							<tr>
								...
								<td class="text-center"><fmt:formatDate
										value="${rental.rent_date}" pattern="yyyy-MM-dd" /></td>
								<td class="text-center"><fmt:formatDate
										value="${rental.ret_exp_date}" pattern="yyyy-MM-dd" /></td>
								<td class="text-center"><c:choose>
										<c:when
											test="${empty rental.ret_act_date && rental.status == 'deleted'}">
											<span class="badge rounded-pill bg-danger status-badge">삭제된
												비품</span>
										</c:when>
										<c:when test="${empty rental.ret_act_date}">
											<span class="badge rounded-pill bg-primary status-badge">대여
												중</span>
										</c:when>
										<c:otherwise>
											<span
												class="badge rounded-pill bg-light text-dark border status-badge">반납
												완료</span>
										</c:otherwise>
									</c:choose></td>
								<td class="text-center"><c:if
										test="${empty rental.ret_act_date}">
										<c:choose>
						...
						<c:if test="${empty myRentals}">
							<tr>
								<td colspan="6" class="text-center py-5 text-muted">현재 대여
									중인 물품이 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
